...
int minDisparities=2,maxDisparities=128,P1=22,P2=300;
int adjust=true;
int sub_pixel=true;
...
int census_weight=3,ad_weight=1;
float gr=2.0;
void init()
{
    rgbImageL = imread("images/wood2/view1.png");
	cvtColor(rgbImageL, grayImageL, COLOR_BGR2GRAY);
	rgbImageR = imread("images/wood2/view5.png");
    ...
    groundTruth =imread("images/wood2/disp1.png",IMREAD_GRAYSCALE);
    cost_array = new float**[imageHeight];
    left_cost = new float**[imageHeight];
    right_cost = new float**[imageHeight];
    up_cost = new float**[imageHeight];
    down_cost = new float**[imageHeight];
    rightimg_cost = new float**[imageHeight];
    ...
    for(int i=0;i<imageHeight;i++)
    {
        ...
        left_cost[i] = new float*[imageWidth];
        right_cost[i] = new float*[imageWidth];
        up_cost[i] = new float*[imageWidth];
        down_cost[i] = new float*[imageWidth];
        rightimg_cost[i] = new float*[imageWidth];
        flags[i]=new bool[imageWidth]{};
        for(int j=0;j<imageWidth;j++)
        {
            cost_array[i][j] = new float[maxDisparities+1]{};
            left_cost[i][j] = new float[maxDisparities+1]{};
            right_cost[i][j] = new float[maxDisparities+1]{};
            up_cost[i][j] = new float[maxDisparities+1]{};
            down_cost[i][j] = new float[maxDisparities+1]{};
            rightimg_cost[i][j] = new float[maxDisparities+1]{};
        }
    }
    ...
    for(int i=0;i<imageHeight;i++)
    {
        for(int j=0;j<imageWidth;j++)
        {
            ...
            for(int d=minDisparities;d<=maxDisparities;d++)
            {
                ...
                cost = float(Hamming32(cencusL,cencusR));
                float costad=abs(grayImageL.at<uchar>(i,j)-grayImageR.at<uchar>(i,j-d));
                //cost=(1-exp(-cost/float(census_weight)));
                //costad=(1-exp(-costad/float(ad_weight)));
                cost=(cost*census_weight+costad*ad_weight/4.0f)/float(census_weight+ad_weight);
                cost_array[i][j][d] = cost;
                }
                else
                {
                    cost_array[i][j][d] = 200000;
                }
...
void compute_disparity()
{
    for(int i=0;i<imageHeight;i++)
    {
        ...
        float cost_1 = min_cost;
        float cost_2 = min_cost;
...
{
for(int i=0;i<imageHeight;i++)
    {   float lastmin=100000;
        for(int j=0;j<imageWidth;j++)
        {   float nowmin=100000;
            for(int d=minDisparities;d<=maxDisparities;d++)
            {
                float cost = cost_array[i][j][d];
                if(j==0)
                {
                ...
                float m1=left_cost[i][j-1][d];
                float m2=100000;
                if(d>0)
                     m2=left_cost[i][j-1][d-1]+P1;
                float m3=100000;
                if(d<maxDisparities)
                     m3=left_cost[i][j-1][d+1]+P1;
                float pp2=max(P2/(abs(int(grayImageL.at<uchar>(i,j))-int(grayImageL.at<uchar>(i,j-1)))+1),P1);
                float m4=lastmin+pp2;
    ...
    {   int lastmin=100000;
        for(int j=imageWidth-1;j>=0;j--)
        {
            ...
            for(int d=minDisparities;d<=maxDisparities;d++)
            {
                ...
                float m1=right_cost[i][j+1][d];
                float m2=100000;
                if(d>0)
                     m2=right_cost[i][j+1][d-1]+P1;
                float m3=100000;
                if(d<maxDisparities)
                     m3=right_cost[i][j+1][d+1]+P1;
                    float pp2=max(P2/(abs(grayImageR.at<uchar>(i,j)-grayImageR.at<uchar>(i,j+1))+1),P1);
                float m4=lastmin+pp2;
    ...
    {   int lastmin=100000;
        for(int i=0;i<imageHeight;i++)
        {
            ...
            for(int d=minDisparities;d<=maxDisparities;d++)
            {
                ...
                float m1=up_cost[i-1][j][d];
                float m2=100000;
                if(d>0)
                     m2=up_cost[i-1][j][d-1]+P1;
                float m3=100000;
                if(d<maxDisparities)
                     m3=up_cost[i-1][j][d+1]+P1;
                    float pp2=P2/(abs(grayImageL.at<uchar>(i,j)-grayImageL.at<uchar>(i-1,j))+1);
                float m4=lastmin+pp2;
    ...
    {   float lastmin=100000;
        for(int i=imageHeight-1;i>=0;i--)
        {
            ...
            for(int d=minDisparities;d<=maxDisparities;d++)
            {
                ...
                float m1=down_cost[i+1][j][d];
                float m2=100000;
                if(d>0)
                     m2=down_cost[i+1][j][d-1]+P1;
                float m3=100000;
                if(d<maxDisparities)
                     m3=down_cost[i+1][j][d+1]+P1;
                    float pp2=P2/(abs(grayImageL.at<uchar>(i,j)-grayImageL.at<uchar>(i+1,j))+1);
                float m4=lastmin+pp2;
...
void my_sgm(int,void*)
{
    ...
    createTrackbar("census_weight","disparity",&census_weight,100,my_sgm);
    createTrackbar("ad_weight","disparity",&ad_weight,100,my_sgm);
