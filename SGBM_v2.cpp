...
int census_weight=1,ad_weight=0;
...
void init()
{
    ...
    temp_cost=new float**[imageHeight];
    ...
    for(int i=0;i<imageHeight;i++)
    {
        ...
        temp_cost[i] = new float*[imageWidth];
        flags[i]=new bool[imageWidth]{};
        for(int j=0;j<imageWidth;j++)
        {
            temp_cost[i][j] = new float[64]{};
            cost_array[i][j] = new float[64]{};
            left_cost[i][j] = new float[64]{};
            right_cost[i][j] = new float[64]{};
            up_cost[i][j] = new float[64]{};
            down_cost[i][j] = new float[64]{};
            rightimg_cost[i][j] = new float[64]{};
        }
    }
    ...
    for(int j=1;j<imageWidth-1;j++)
    {
        ...
        {   
            ...
            else
            {
                ...
                temp_cost[i][j][d]=((gray_cost+sobel_cost)*0.5f)*0.5f;
...
{   int total=imageHeight*imageWidth;
    ...
    for(int i=0;i<imageHeight;i++)
    {
        for(int j=0;j<imageWidth;j++)
        {
            float truth=float(groundTruth.at<uchar>(i,j))/4.0f;
            float dis;
            if(adjust)
            {
